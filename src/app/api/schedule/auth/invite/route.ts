...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!requester.isAuthenticated || (requester.role !== 'owner' && requester.role !== 'manager')) {
      return jsonResponse({ 
        error: `Access denied. Managers or owners only. (Details: email=${requester.email}, role=${requester.role}, auth=${requester.isAuthenticated}, err=${(requester as any).authError || 'none'})` 
      }, 403);
    }
